...
Radar.prototype._addOneRiskPoint = function(options) {
  ...
  elements_group.innerHTML = '<div class="dashed-circle"></div>\
                              <div class="red-flag" data-city="' + options.cityName + '"></div>\
                              <div class="info-panel">\
                                  <div class="info-title">' + options.title + '</div>\
                                  <div class="info-content">\
                                      <div>\
                                          地区 ：<span class="risk-city">' + options.city + '</span>\
                                      </div>\
                                      <div>\
                                         医院数量 ：<span class="risk-num">' + '' + '</span>\
                                      </div>\
                                  </div>\
                              </div>';
...
document.querySelector('#radar-switch-btn').addEventListener('click', function(){
  radar.roamingToggle();
});
var key = JSON.stringify({bizId:'bamp'});
// (function(){
//   $.ajax({
//       type:'post',
//       dataType:'json',
//       url:parent.getUrl()+ 'audit/nodeinfo.do',
//       beforeSend:function(xhr){
//           xhr.setRequestHeader("token",parent.getToken());
//       },
//       success:function(data){
//           var info = data.data;
//           if(data.success){
//               var point = JSON.parse(info);
//               radar.addRiskPoints(renderRiskPoint(point));
//           }else{
//               parent.errorReact(info)
//           }
//       },error:function(data){
//           console.log(data);
//       }
...
const arr = [
    {
      ...
      'port':
        [
          { 'port': '5001', 'status': true },
          { 'port': '5002', 'status': true }
        ],
      'ms': '1 ms', 'ip': '172.16.124.52',
      'name': '', 'nodeid': 'beat-shie-ep1', 'height': 737
    },
    {
      'owner': '',
      'port': [
        { 'port': '5003', 'status': true },
        { 'port': '5004', 'status': true }],
      'ms': '1 ms',
      'ip': '172.16.124.52',
      'name': '',
      'nodeid': 'beat-account-ep1', 'height': 737
    },
    {
      'owner': '',
      'port': [
        { 'port': '5005', 'status': true },
        { 'port': '5006', 'status': true }
      ],
      'ms': '0 ms',
      'ip': '172.16.124.52',
      'name': '',
      'nodeid': 'beat-trustee-ep1',
      'height': 737
    },
    {
      'owner': '',
      'port': [
        { 'port': '5007', 'status': true },
        { 'port': '5008', 'status': true }
      ],
      'ms': '0 ms',
      'ip': '172.16.124.52',
      'name': '',
      'nodeid': 'beat-bank-ep1', 'height': 737
    },
    {
      'owner': '',
      'port': [
        { 'port': '5009', 'status': true }, { 'port': '5010', 'status': true }],
      'ms': '0 ms',
      'ip': '172.16.124.52',
      'name': '',
      'nodeid': 'beat-etrustee-ep1',
      'height': 737
    }, {
      'owner': '',
      'port': [{ 'port': '5011', 'status': true }, { 'port': '5012', 'status': true }],
      'ms': '0 ms',
      'ip': '172.16.124.52',
      'name': '',
      'nodeid': 'beat-hrss-ep1',
      'height': 737
    }, {
      'owner': '',
      'port': [{ 'port': '6003', 'status': true }],
      'ms': '1 ms',
      'ip': '172.20.14.73',
      'name': '',
      'nodeid': 'beat-orderer',
      ...
      'owner': '',
      'port': [{ 'port': '5013', 'status': true }, { 'port': '5014', 'status': true }],
      'ms': '0 ms',
      'ip': '172.16.124.52',
      'name': '',
      'nodeid': 'beat-hrssb-ep1',
      'height': 737
    }
]
var point = arr
...
function renderRiskPoint(point){
  var nodePoint = [];
  var adlr = {};
  var bdlr = {};
  var str = {};
  ...
  for(var i = 0;i <9;i++){
      var obj = {};
      obj.title = point[i].name;
      obj.total = point[i].height;
      if(point[i].nodeid=="beat-hrss-ep1"){
        obj.cityName = '';
        obj.coords = [108.95000,34.26667]
      }else if(point[i].nodeid=="beat-hrssb-ep1"){
        obj.cityName = '';
        obj.coords = [119.30000,26.08333]
      }else if(point[i].nodeid=="beat-trustee-ep1"){
        obj.cityName = '';
         obj.coords = [121.47333,31.50000]
      }else if(point[i].nodeid=="beat-account-ep1"){
        obj.cityName = '';
        obj.title = "";
        obj.coords = [106.45000,29.56667]
      }else if(point[i].nodeid=="beat-bank-ep1"){
        obj.cityName = '';
        obj.title = "";
        obj.coords = [113.23333,23.16667]
      }else if(point[i].nodeid=="beat-etrustee-ep1"){
        obj.cityName = '';
        obj.coords = [118.78333,32.05000]
      }
      var j = point[i].port.length - 1;
      if(point[i].port[j].status ==true){
          obj.type = '';
          obj.city = '';
      }else{
          obj.type = '';
  ...
  nodePoint.push(zgr);
  nodePoint.push(tgr);
  nodePoint.push(qtstr);
  return nodePoint;
}
var i = -1;
function randomLocation(){
  i++;
  var pointList = [[116.41667,39.91667],
      [113.23333,23.16667],
      [106.45000,29.56667],
      [108.95000,34.26667],
      [118.78333,32.05000],
...
//    var staticColor = '#000',sendColor='#FFD700',receiveColor='#bf465b',radioColor = '#009688';
var staticColor = '#AE0000',sendColor='#FFD700',receiveColor='#bf465b',radioColor = '#009688';
  function final(List){
      var dom = document.getElementById("container");
      var myChart = echarts.init(dom);
      var City,Line;
      if(List){
          City = List.defaultCity;
          Line = List.defaultLine;
      }else{
          City = [
              {name:'上海',value:[121.4648,31.2891, -1],symbolSize:10,itemStyle:{normal:{color:staticColor}},label:{normal:{textStyle:{fontSize:18}}}},
              {name:'浙江',value:[120.15,30.28, -1],symbolSize:10,itemStyle:{normal:{color:staticColor}},label:{normal:{textStyle:{fontSize:18}}}},
              {name:'湖北',value:[114.30,30.60, -1],symbolSize:10,itemStyle:{normal:{color:staticColor}},label:{normal:{textStyle:{fontSize:18}}}},
              {name:'北京',value:[116.3048,39.9916, 11],symbolSize:10,itemStyle:{normal:{color:staticColor}},label:{normal:{textStyle:{fontSize:18}}}},
              {name:'江苏',value:[118.78,32.07, 11],symbolSize:10,itemStyle:{normal:{color:staticColor}},label:{normal:{textStyle:{fontSize:18}}}},
          ];
          Line =[];
      }
      var option = {
  ...
  final();
  function dataArrangement(data){
      var point = {};
      // var defaultCity = [
  // {name:'受托人(长江养老)',value:[121.4648,31.2891, -1],symbolSize:10,itemStyle:{normal:{color:staticColor}},label:{normal:{textStyle:{fontSize:18}}}},
  // {name:'账管人A',value:[107.7539,30.1904, -1],symbolSize:10,itemStyle:{normal:{color:staticColor}},label:{normal:{textStyle:{fontSize:18}}}},
  // {name:'托管人A',value:[113.2333,23.1666, -1],symbolSize:10,itemStyle:{normal:{color:staticColor}},label:{normal:{textStyle:{fontSize:18}}}},
  // {name:'其他受托人A',value:[116.3048,39.9916, 11],symbolSize:10,itemStyle:{normal:{color:staticColor}},label:{normal:{textStyle:{fontSize:18}}}},
  // {name:'A省代理人',value:[108.9500,34.2666, 11],symbolSize:10,itemStyle:{normal:{color:staticColor}},label:{normal:{textStyle:{fontSize:18}}}},
  //   {name:'B省代理人',value:[119.1800,26.0500, 11],symbolSize:10,itemStyle:{normal:{color:staticColor}},label:{normal:{textStyle:{fontSize:18}}}}
  //     ];
      var defaultLine = [],fromLocation,radio = [];
      for(var i = 0; i < defaultCity.length;i++){
          if(data.fromCode===defaultCity[i].name){
              defaultCity[i].itemStyle.normal.color = sendColor;
              fromLocation = defaultCity[i].value;
              radio.push(defaultCity[i].name);
          }
      }
      for(var i = 0; i < defaultCity.length;i++){
          for(var j = 0;j < data.toCode.length;j++){
              var obj = {},toLocation;
              obj.fromName = data.fromCode;
              obj.toName = data.toCode[j];
              if(data.toCode[j]==defaultCity[i].name){
                  defaultCity[i].itemStyle.normal.color = receiveColor;
                  toLocation = defaultCity[i].value;
                  obj.coords = [fromLocation,toLocation];
                  defaultLine.push(obj);
                  radio.push(defaultCity[i].name);
              }
          }
      }
      var city = [];
      for(var i = 0;i <defaultCity.length;i++){
          city.push(defaultCity[i]);
      }
      for(var i = 0;i <radio.length;i++){
          for(var j = 0;j <city.length;j++){
              if(radio[i]==city[j].name){
                  city.splice(j,1);
              }
          }
      }
      for(var i = 0;i <defaultCity.length;i++){
          for(var j = 0;j <city.length;j++){
              var obj = {};
              obj.fromName = data.fromCode;
              obj.toName = city[j].name;
              if(defaultCity[i].name==city[j].name){
                  defaultCity[i].itemStyle.normal.color = radioColor;
                  toLocation = defaultCity[i].value;
                  obj.coords = [fromLocation,toLocation];
                  defaultLine.push(obj);
              }
      ...
      point.defaultCity = defaultCity;
      point.defaultLine = defaultLine;
      return point;
  }
//   var webSocket = new WebSocket('ws://'+parent .getUrl(1)+'welcome');
//   webSocket.onopen = function(){
//       webSocket.send(1);
//   };
//   webSocket.onmessage = function(evt){
//       var data =JSON.parse(evt.data),i = 0;
//       console.log(1,data);
//       var timer = setInterval(function(){
//           i++;
//           final(dataArrangement(data));
//           setTimeout(function(){
//               final();
//           },3000);
//           if(i >= 3){
...
$('#changeView').click(function(event){
  ...
  if(status=='none'){
      $('#changeView').find('span').text('切换节点监控');
      $('#container').css('display','block');
      $('#radarL').css('display','none');
      dataMonitoring();
  }else{
      $('#changeView').find('span').text('切换数据监控');
      $('#container').css('display','none');
      $('#radarL').css('display','block');
  }
});
